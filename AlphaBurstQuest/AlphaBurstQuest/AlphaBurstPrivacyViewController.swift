...
class AlphaBurstPrivacyViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    
    @IBOutlet weak var backButton: UIButton!
    @IBOutlet weak var indicatorView: UIActivityIndicatorView!
    @IBOutlet weak var alphaBurstWebView: WKWebView!
    @IBOutlet weak var topCos: NSLayoutConstraint!
    @IBOutlet weak var bottomCos: NSLayoutConstraint!
    var backAction: (() -> Void)?
    var privacyData: [Any]?
    @objc var url: String?
    let privacyPolicyUrl = "https://www.termsfeed.com/live/4a07656c-fb75-4d2e-a8c5-bf71d6879bc5"
    
    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.alphaBurstGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        alphaBurstStartLoadWebView()
    }
    ...
    private func initSubViews() {
        alphaBurstWebView.scrollView.contentInsetAdjustmentBehavior = .never
        view.backgroundColor = .black
        alphaBurstWebView.backgroundColor = .black
        alphaBurstWebView.isOpaque = false
        alphaBurstWebView.scrollView.backgroundColor = .black
        indicatorView.hidesWhenStopped = true
    }
    
    private func initNavView() {
        guard let url = url, !url.isEmpty else {
            alphaBurstWebView.scrollView.contentInsetAdjustmentBehavior = .automatic
            return
        }
    ...
    private func initWebView() {
        ...
        let userContentC = alphaBurstWebView.configuration.userContentController
        ...
        alphaBurstWebView.navigationDelegate = self
        alphaBurstWebView.uiDelegate = self
    }
    
    
    private func alphaBurstStartLoadWebView() {
        let urlStr = url ?? privacyPolicyUrl
        guard let url = URL(string: urlStr) else { return }
        
        indicatorView.startAnimating()
        let request = URLRequest(url: url)
        alphaBurstWebView.load(request)
    }
    
    private func alphaBurstReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "AlphaBurstPrivacyViewController") as? AlphaBurstPrivacyViewController {
                adView.url = adurl
                adView.backAction = { [weak self] in
                    let close = "window.closeGame();"
                    self?.alphaBurstWebView.evaluateJavaScript(close, completionHandler: nil)
                }
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                alphaBurstSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                alphaBurstReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        alphaBurstSendEvent(tName, values: [tName: data])
                    }
                } else {
                    alphaBurstSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                alphaBurstAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    alphaBurstAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = alphaBurstJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = alphaBurstJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    alphaBurstSendEvents(withParams: evParams)
                }
            }
        }
    }
